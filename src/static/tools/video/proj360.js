...
(function (global) {
  ...
  var FS = [
    ...
    'uniform mat3 uCorr;',                             // 水平補正（元の球をどう回すか）
  ...
  function create(W, H, canvasEl) {
    ...
    var U = {};
    ['uTex', 'uYaw', 'uPitch', 'uFov', 'uAspect', 'uCorr'].forEach(function (n) { U[n] = gl.getUniformLocation(prog, n); });
    ...
    /* corr: 行優先 3x3（配列9つ）。省略なら補正なし */
    function draw(src, yaw, pitch, fov, corr) {
      if (src) upload(src);
      gl.viewport(0, 0, canvas.width, canvas.height);
      gl.uniform1f(U.uYaw, yaw || 0);
      gl.uniform1f(U.uPitch, pitch || 0);
      gl.uniform1f(U.uFov, fov || (80 * Math.PI / 180));
      gl.uniformMatrix3fv(U.uCorr, false, colMajor(corr && corr.length === 9 ? corr : I3));
